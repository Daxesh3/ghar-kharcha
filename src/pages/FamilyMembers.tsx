    ...
    return (
        <div className='max-w-4xl mx-auto py-6'>
            <div className='mb-6 flex justify-between items-center'>
                ...
                <button onClick={() => setIsAdding(true)} className='btn btn-primary flex items-center' disabled={isAdding}>
            ...
            {isAdding && (
                <div className='card p-4 mb-6 border-primary-200 border-2'>
                    ...
                    <form onSubmit={handleAddSubmit}>
                        <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
                            ...
                            <div>
                                ...
                                <select id='role' name='role' className='input-field' value={formData.role} onChange={handleInputChange} required>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                {familyMembers.length === 0 ? (
                    <div className='card p-8 text-center md:col-span-2 lg:col-span-3'>
                        ...
                        <p className='text-gray-500 mb-4'>Add family members to track expenses for each person</p>
                        <button onClick={() => setIsAdding(true)} className='btn btn-primary inline-flex items-center'>
                            <Plus size={18} className='mr-1.5' />
                            Add First Member
                        </button>
                    </div>
                        ...
                        <div key={member.id} className='card p-5'>
                            {editingId === member.id ? (
                                <form onSubmit={handleEditSubmit} className='space-y-4'>
                                    <div>
                                        <label htmlFor={`edit-name-${member.id}`} className='input-label'>
                                            Name
                                        </label>
                                        <input
                                            type='text'
                                    ...
                                    <div>
                                        <label htmlFor={`edit-role-${member.id}`} className='input-label'>
                                            Role
                                        </label>
                                        <select
                                    ...
                                    <div>
                                        <label htmlFor={`edit-avatar-${member.id}`} className='input-label'>
                                            Avatar URL
                                        </label>
                                        <input
                                            type='url'
                                    ...
                                    <div className='flex justify-between'>
                                        <button type='button' onClick={cancelEditing} className='btn btn-secondary flex items-center'>
                                            <X size={16} className='mr-1.5' />
                                            Cancel
                                        </button>
                                        <button type='submit' className='btn btn-success flex items-center'>
